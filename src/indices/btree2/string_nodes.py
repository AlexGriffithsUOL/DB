...
class BTreeNode(Page):
    ...
    @property
    def data_end(self):
        return int.from_bytes(bytes=self.data[self.DATA_END_OFFSET[0]:self.DATA_END_OFFSET[1]], byteorder='little', signed=False)
    
    @data_end.setter
    def data_end(self,value):
        encoded = int.to_bytes(value, 2, 'little', signed=False)
        self.write_data(*self.DATA_END_OFFSET, encoded)
    ...
    def available_data(self):
        return len(self.data) - self.DATA_START_IDX
    
    def size_key(self, value):
        return len(self.datatype.serialise(value))
    
    @property
    def pointer_block_size(self):
        if self.is_leaf:
            return self.PAGE_PTR_SIZE + self.SLOT_PTR_SIZE
        else:
            return self.PAGE_PTR_SIZE
        
    @property
    def key_pointer_block_size(self):
        return self.key_size + self.pointer_block_size
    ...
    def deserialise(self):
        keys = []
        pointers = []
        starting_pointer = self.DATA_START_IDX
        
        if self.is_leaf:
            
            for i in range(self.number_of_keys):
                
                key_length_start = starting_pointer
                key_length_end = key_length_start + 2
                key_start = key_length_end
                key_length = int.from_bytes(bytes=self.data[key_length_start : key_length_end], byteorder='little', signed=False)
                key_end = key_start + key_length
                page_start = key_end
                page_end = page_start + self.PAGE_PTR_SIZE
                slot_start = page_end
                slot_end = slot_start + self.SLOT_PTR_SIZE
                starting_pointer = slot_end
                
                
                key = self.deserialise_key(key_start, key_end)
                page_pointer = self.deserialise_ptr(page_start, page_end)
                slot_pointer = self.deserialise_ptr(slot_start, slot_end)
            ...
            for i in range(self.number_of_keys):
                ...
                page_start = starting_pointer
                page_end = page_start + self.PAGE_PTR_SIZE
                key_length_start = page_end
                key_length_end = key_length_start + 2
                key_start = key_length_end
                key_length = int.from_bytes(self.data[key_length_start:key_length_end], 'little', signed=False)
                key_end = key_start + key_length
                
                key = self.deserialise_key(key_start, key_end)
                pointer = self.deserialise_ptr(page_start, page_end)
                
                starting_pointer = key_end
    ...
    def save(self):
        if self.is_leaf:
            ...
            for i in range(self.number_of_keys):
                key_length_start = starting_pointer
                key_length_end = key_length_start + 2
                key_start = key_length_start # THE DATATYPE HANDLES THE LENGTH
                key_length = self.size_key(self.keys[i])
                key_end = key_start + key_length
                page_start = key_end
                page_end = page_start + self.PAGE_PTR_SIZE
                slot_start = page_end
                ...
                key = self.serialise_key(self.keys[i]) # includes 2-byte wide length
                
                rid = self.pointers[i]
                page_ptr = self.serialise_ptr(rid[0], self.PAGE_PTR_SIZE)
                slot_ptr = self.serialise_ptr(rid[1], self.SLOT_PTR_SIZE)
                
                self.write_data(key_length_start, key_end, key) # lost 2 in here
                self.write_data(page_start, page_end, page_ptr)
                self.write_data(slot_start, slot_end, slot_ptr)
            ...
            for i in range(self.number_of_keys):
                # page_pointer_start = self.DATA_START_IDX + (i * self.key_pointer_block_size) # need to fix this to make use of our cycle method
        ...
        self.data_end = starting_pointer
        self.flush()
    ...
    def _insert_leaf_(self,key,rid):
        ...
        if self.data_end + self.size_key(key) + self.PAGE_SLOT_PTR_SIZE >= len(self.data):
